...
const TaskList = () => {
    ...
    //             headers: { Authorization: `Bearer ${user}` },
    //         });
    //         console.log('Task removed:', response.data); // Check the response from the server
    
    //         // Update tasks state after successful deletion
    //         setTasks((prevTasks) => prevTasks.filter((task) => task._id !== taskId));
    //     } catch (error) {
    //         console.error('Error removing task:', error);
    //         alert('Error removing task'); // Show an alert if there's an error
    //     }
    // };
    ...
    const handleUpdate = async () => {
        try {
            ...
            setTasks((prevTasks) =>
                prevTasks.map((task) => (task._id === editingTask ? updatedTask.data : task))
            );
            setEditingTask(null); // Clear editing mode
        } catch (error) {
            console.error('Error updating task:', error);
                    ...
                    <li key={task._id}>
                        {editingTask === task._id ? (
                            ...
                            <div>
                                ...
                                <button onClick={() => handleEdit(task)}>Edit</button>
                                {/* <button onClick={() => { console.log(task._id); handleRemove(task._id); }}>Delete</button> */}
